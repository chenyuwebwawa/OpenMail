  ...
  async req(method, path, body) {
    ...
    if (res.status === 401 && !path.startsWith('/auth/')) {
      ...
      if (location.hash.startsWith('#/login')) {
        location.reload(); // 已在登录路由时强制重绘，避免残留旧界面
      } else {
        location.hash = '#/login';
      }
      throw new Error(json.error || '请重新登录');
    }
...
const I = {
  ...
  chart: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="m7 15 4-6 4 4 5-8"/></svg>',
  ...
  junk: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m4.9 4.9 14.2 14.2"/></svg>',
  ...
  draft: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>',
  ...
  alert: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 20h16a2 2 0 0 0 1.73-2Z"/><path d="M12 9v4M12 17h.01"/></svg>',
...
export const icon = (name) => I[name] || '';

// ---------- Logo ----------
export const logoSvg = (size = 30) => `<svg class="logo-mark" width="${size}" height="${size}" viewBox="0 0 100 100"><rect width="100" height="100" rx="22" fill="#4f6ef7"/><path d="M22 32 L50 56 L78 32" stroke="white" stroke-width="9" fill="none" stroke-linecap="round" stroke-linejoin="round"/><rect x="22" y="30" width="56" height="40" rx="8" stroke="white" stroke-width="7" fill="none"/></svg>`;
...
const AV_COLORS = ['#4f6ef7', '#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#06b6d4', '#ef4444', '#6366f1'];
...
export function avatarHtml(name, cls = '') {
  ...
  const initials = [...n].slice(0, 1).join('').toUpperCase() || '?';
  return `<span class="avatar ${cls}" style="background:${avatarColor(n)}">${esc(initials)}</span>`;
}
export function displayNameOf(m) {
  return m.from_name || (m.from_addr || '').split('@')[0] || '?';
}
...
export function modal({ title, body, footer, wide = false, onClose }) {
  ...
  mask.innerHTML = `
    <div class="modal ${wide ? 'wide' : ''}">
      <div class="modal-head"><h3>${esc(title)}</h3><button class="btn btn-ghost btn-icon" data-close>${icon('x')}</button></div>
  ...
  return { mask, close, el: mask.querySelector('.modal') };
}
